...
| Removing a `QShortcut`, or the font pass in `apply_theme_and_font` | Don't, without measuring teardown 10+ times | This app's Qt teardown is fragile in ways nobody has isolated. Dropping the font pass crashed 6 runs of 6; folding three `QShortcut`s into the event filter crashed 12 of 12 — both pure cleanup, both reverted. Measure with a subprocess that constructs the window and exits, under a real X server. |
| User preference that resets each launch | Store it in `QSettings` and read it defensively | Values come back as strings on the INI backend, and a settings file outlives the layout that wrote it — validate before applying (see `restore_window_layout`). |
